...
/* flexspi_setup_clock run in RAM when XIP. */
void flexspi_setup_clock(XSPI_Type *base, uint32_t src, uint32_t divider)
{
	if (base == XSPI0) {
...
{
	flexspi_setup_clock(XSPI0, 0U, 1U);
	flexspi_setup_clock(XSPI1, 0U, 1U);
}
