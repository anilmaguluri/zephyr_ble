...
volatile uint32_t xlnx_tmrctr_read_hw_cycle_count(const struct device *dev)
{
	...
	return (CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC / config->clock_rate) *
				 xlnx_tmrctr_read_count(dev);
}
...
static void xlnx_tmrctr_irq_handler(const struct device *dev)
{
	...
	const struct xilinx_timer_config *config = dev->config;
	uint32_t cycles = xlnx_tmrctr_read_hw_cycle_count(dev);

	LOG_DBG("%s: Base 0x%lx IRQ happened\n", __func__, config->base);

	/*
	 * TODO: CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC is likely all the time equal to timer
	 * frequency that's why pretty much all calculation around it should be fixed.
	 */
...
static int xilinx_timer_init(const struct device *dev)
{
	...
	if (!sys_dev && !config->one_timer) {
		...
		xlnx_tmrctr_set_reset_value(dev, data->clockevent_offset,
					    (config->clock_rate / CONFIG_SYS_CLOCK_TICKS_PER_SEC) -
					    1UL);
